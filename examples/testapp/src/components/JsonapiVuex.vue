<template>
    <div id="main-div">
        ...
        <div id="put">
            ...
            <div v-if="'_jv' in widget1 && 'rels' in widget1['_jv']">
                <div v-for="(value, name, index) in widget1['_jv'].attrs" :key="index">
                    <label :for="'put_' + name">{{ name }}</label>
                    <input :id="'put_' + name" v-model="widget1[name]"/>
                </div>
            </div>
            <div>
                <button name="patch_button" @click="putRecord(widget1)">Patch</button>
            </div>
        </div>
        <div id="post">
            <h2>Post widget</h2>
...
<script>
    export default {
        ...
        methods: {
            putRecord(record) {
                this.$store.dispatch('jv/put', [record, {url: 'example'}])
            },
            patchRecord(record) {
